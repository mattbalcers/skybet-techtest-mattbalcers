...
function renderSelection($container, $selection) {
    ...
    $selection.find('.selection__title').text(
        sel.event + ' - ' + sel.name + ' @ ' + formatOdds(sel.odds));
    $selection.find('.selection__returns').text(' Potential Return £' +
...
function renderReceipt($container, $receipt) {
    ...
    $container.append($receipt).toFixed(2);
}
...
$app.on('click', '.js-toggle-b', function (e) {
    e.preventDefault();
    $(this).parent().find('.js-toggle-target-b').toggleClass('is-hidden');
});

/*** if($(".js-toggle-target").is('is-hidden'))
  {
	$('i:active').show(); }
	else
{	$('i').show();
